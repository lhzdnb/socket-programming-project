...
//        cout << "socket() is OK!" << endl;
    ...
    if (bind(udpSocket, (struct sockaddr*)&udpService, sizeof(udpService)) == -1) {
        ...
        cout << "Server H is up and running using UDP on port " << udp_port << "." << endl;
    }
    ...
    while (true) {
        ...
        if (recvBytes < 0) {
            ...
            if (recvBuffer[recvBytes - 2] == 'A') {
                admin = true;
                cout << "Server H received an inventory status request for code " << bookCode << "." << endl;
            }
            else {
        ...
        if (sendBytes < 0) {
            ...
            cout << "Server H finished sending the inventory status to the Main Server using UDP on port " << udp_port << "." << endl;
        }
        else {
